...
def readVideo():
    ...
    inpImage = cv2.VideoCapture(os.path.join(
        CWD_PATH, 'src/server/tools/' + '001.mp4'))
...
def addText(img, radius, direction, deviation, devDirection, sensorDist):
    ...
    deviation_text = 'Deviation: ' + \
        str(round(abs(deviation), 3)) + 'm ' + devDirection
    cv2.putText(img, deviation_text, (50, 600),
                font, 4, (0, 0, 255), 5, cv2.LINE_AA)

    # cv2.putText(img, "Distance: " + sensorDist + "cm", (50, 800), font, 4, (0, 0, 255), 5, cv2.LINE_AA)
...
server_socket.bind(video_stream_address)

server_socket.listen(1)
print('Listening for camera connection...')
...
while True:
    ...
    recv_bytes = b''
    recv_bytes += connection.read(image_len)

    # Read an image from buffer in memory
    newImage = cv2.imdecode(np.fromstring(
        recv_bytes, dtype=np.uint8), cv2.IMREAD_COLOR)

    frames += 1

    ###############################################################

    # frame = np.invert(newImage)
    frame = cv2.bitwise_not(cv2.resize(newImage, dim, interpolation = cv2.INTER_AREA))
    # frame = newImage

    # re, frame = video_capture.read()
    print("ak: " + str(frame.shape))
    # cv2.imshow('Raw Video Frame', frame)
    ...
    dataValues = addText(imread('src/server/tools/white.png'),
                         curveRad, curveDir, deviation, directionDev, "N/A")
    cv2.imshow('data values', cv2.resize(dataValues, (650, 325)))
